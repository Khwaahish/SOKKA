...
class Profile(models.Model):
    ...
    latitude = models.DecimalField(max_digits=10, decimal_places=7, null=True, blank=True, help_text="Latitude for commute radius filtering")
    longitude = models.DecimalField(max_digits=10, decimal_places=7, null=True, blank=True, help_text="Longitude for commute radius filtering")
    commute_radius = models.PositiveIntegerField(null=True, blank=True, help_text="Preferred commute radius in miles (e.g., 10 for 10 miles)")
...
class ProfileSkill(models.Model):
    ...
    def __str__(self):
        profile_name = self.profile.get_full_name() if self.profile else 'Unknown'
        return f"{profile_name} - {self.skill.name} ({self.proficiency_level})"
